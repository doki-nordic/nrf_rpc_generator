...
function normalizeGenBlock(block)
{
	...
	for (let line of lines) {
		let indent = 1;
		[...line.match(/^[\\@\s]*/)[0]].forEach(x => indent += (x == '@' ? 1 : x == '\\' ? -1 : 0));
		line = line.replace(/^[\\@\s]*/, '').trim();
		if (line == '') continue;
		code += options.indentString.repeat(indent) + line + '\n';
		maxLen = Math.max(maxLen, options.indentSize * indent + line.length);
	}
	return [code, maxLen];
}
...
function addSideData(type, block, maxLineLength)
{
	...
	block = block.replace(/(?<=^|\n)(.*?)\n/g, (_, line) => {
		let indent = 0;
		let lineNoInd = line;
		while (lineNoInd.startsWith(options.indentString)) {
			indent++;
			lineNoInd = lineNoInd.substr(options.indentString.length);
		}
		let spaces = maxLineLength - lineNoInd.length - options.indentSize * indent;
...
function generate(genBlocks, userBlocks, blockOrder)
{
	let output = '';
	...
	for (let name of blockOrder) {
		if (name in genBlocks && genBlocks[name].trim() != '') {
			output += addSideData(name, genBlocks[name], maxLineLength);
			output += '\n';
		}
		delete genBlocks[name];
		if (name in userBlocks && userBlocks[name].trim() != '') {
			output += userBlocks[name];
			output += '\n';
		}
		delete userBlocks[name];
	}

	if (Object.keys(genBlocks).length > 0)
	{
		throw Error(`Internal: Following code blocks were generated, but not put into output: ${Object.keys(genBlocks).join(', ')}`);
	}

	for (let name in userBlocks) {
		//TODO: warning
		output += `#warning Following part of user code was not fitting into any block after regeneration. Fix it manually.\n`;
		output += `#if 0\n`;
		output += userBlocks[name];
		output += '\n#endif\n';
	}
