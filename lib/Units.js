...
class UnitStruct {
	...
	constructor(module, func, custom) {
		...
		this.fields = {};
		for (let [name, type, isInUnion] of this.struct.getFields()) {
		...
		for (let m of this.encFunc.getSerializeMacros('DEL')) {
			delete this.fields[m.value];
		}

		for (let m of this.encFunc.getSerializeMacros('UNION_SELECT')) {
			this.fields[m.value].isInUnion = false;
		}

		for (let f of Object.values(this.fields)) {
			if (f.isInUnion) {
				throw Error(`Field '${f.name}' in structure '${this.type}' is inside a union.`);
			}
		}
