...
class UnitFunc {
	...
	_collectSymbols() {
		...
		this.rspFunc = this.parser.functions[this.name + RSP_FUNC_POSTFIX];
		...
		this.resStruct = this.parser.structures['struct ' + this.name + RES_STRUCT_POSTFIX];

		if (!this.resStruct) {
			this.resStructPlaceholder = this.rspFunc ? this.rspFunc.placeholderBefore() : this.rspFuncPlaceholder.placeholderBefore();
		}

		this.recvFunc = this.parser.functions[this.name + RECV_FUNC_POSTFIX];
	...
	generate() {
		...
		let blocks = generator.generateCliSendFunc();
		if (this.sendFunc.defined) {
		...
		if (this.responseHandlerGenerate) {
			blocks = generator.generateCliRspFunc();
			if (this.rspFunc) {
				this.rspFunc.regenerate(blocks, true);
			} else {
				this.rspFuncPlaceholder.regenerate(blocks);
			}
			blocks = generator.generateCliResStruct();
			if (this.resStruct) {
				this.resStruct.regenerate(blocks, true);
			} else {
				this.resStructPlaceholder.regenerate(blocks);
			}
		} else {
			if (this.rspFunc) {
		...
		blocks = generator.generateHostRecvFunc();
		if (this.recvFunc) {
			this.recvFunc.regenerate(blocks, true);
		} else {
			this.recvFuncPlaceholder.regenerate(blocks);
		}
