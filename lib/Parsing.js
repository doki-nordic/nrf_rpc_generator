...
class Struct {
	...
	*_getFields(parentNode, prefix, isUnion) {

		let lastRecordDecl = null;

		isUnion = isUnion || parentNode.tagUsed == 'union';

		for (let node of parentNode.inner) {
			if (node.kind == 'RecordDecl') {
				lastRecordDecl = node;
			} else if (node.kind != 'FieldDecl') {
				// skip
			} else if (node.type.qualType.indexOf('at ') < 0) {
				yield [`${prefix}${node.name}`, node.type.qualType.trim(), isUnion];
			} else if (!node.name) {
				for (let x of this._getFields(lastRecordDecl, prefix, isUnion))
					yield x;
			} else {
				for (let x of this._getFields(lastRecordDecl, `${prefix}${node.name}.`, isUnion))
					yield x;
			}
		}
	...
	*getFields() {
		...
		for (let x of this._getFields(this._node, '', false)) {
			yield x;
		}
	}
...
class Var {
	...
	add(node, side, src) {
		...
		this._node = node;
		this._side = node.loc.isInput ? side : OTHER;
		this._src = node.loc.isInput ? src : null;
	}

	getValue() {
		let node = findRecursive(this._node, inner => (inner.kind == 'IntegerLiteral'));
		if (!node) {
			throw Error(`Cannot get value of ${this.name}.`);
		}
...
class Parser {
	...
	constructor() {
		...
		this._collectSymbols(this.cli._ast, CLIENT, this.cli.functions, this.cli.structures, this.cli.callbacks, this.cli.vars);
		this._collectSymbols(this.host._ast, HOST, this.host.functions, this.host.structures, this.host.callbacks, this.host.vars);
		...
		this.commonVars = this._collectCommon(this.cli.vars, this.host.vars);
	...
	_collectSymbols(ast, side, functions, structures, callbacks, vars) {

		this._collectItems(ast, functions, side, Func, '', node =>
			node.kind == 'FunctionDecl' && 
			!findRecursive(node, inner => (
				inner.kind == 'StringLiteral' &&
				inner.value == '"__SERIALIZE__IGNORE__"'
			))
		);
		...
		this._collectItems(ast, callbacks, side, Callback, '', node =>
			...
			'inner' in node &&
			node.inner.some(node2 =>
				node2.kind == 'FunctionProtoType'
			)
		);

		this._collectItems(ast, vars, side, Var, '', node =>
			node.kind == 'VarDecl' &&
			!node.name.startsWith('_serialize_unique_') &&
			'inner' in node &&
			(
				node.inner.some(node2 =>
					node2.kind == 'IntegerLiteral'
				) ||
				node.inner.some(node2 =>
					node2.kind == 'ImplicitCastExpr' &&
					'inner' in node2 &&
					node2.inner.some(node3 =>
						node3.kind == 'IntegerLiteral'
					)
				)
			)
		);
	...
	_parseFile(file, specificParams) {
		...
		try {
			...
			cmd = cmd.replace('${include-stubs}', `${options.tmpDir}/include`);
