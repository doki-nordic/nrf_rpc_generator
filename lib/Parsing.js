...
class Parser {
	...
	constructor(inputFile) {
		...
		if (inputSide == CLIENT) {
			...
			[this._hostAst, this._hostSrc] = this._parseFile(this._hostSrcPath, this._clangHostParams);
		} else {
			...
			[this._cliAst, this._cliSrc] = this._parseFile(this._cliSrcPath, this._clangCliParams);
		}

		this._collectSymbols(this._cliAst, CLIENT, this.cliFunctions, this.cliStructures, this.cliCallbacks);
		this._collectSymbols(this._hostAst, HOST, this.hostFunctions, this.hostStructures, this.hostCallbacks);
		this.commonFunctions = this._collectCommon(this.cliFunctions, this.hostFunctions);
		this.commonStructures = this._collectCommon(this.cliStructures, this.hostStructures);
		this.commonCallbacks = this._collectCommon(this.cliCallbacks, this.hostCallbacks);
	...
	_collectItems(ast, collection, side, Class, namePrefix, callback) {
		...
		for (let node of list) {
			...
			collection[name].add(node, side, side == CLIENT ? this._cliSrc : this._hostSrc);
		}
	}
	...
	placeholderOnHost() {
		return new Placeholder(this._hostSrc.length, this._hostSrc, true);
	}
};
