...
let predefinedResponseHandlers = {
	'bool': ['ser_rsp_decode_bool', 'ser_rsp_send_bool'],
	'float': ['ser_rsp_decode_float', 'ser_rsp_send_float'],
	'double': ['ser_rsp_decode_double', 'ser_rsp_send_double'],
	'int8_t': ['ser_rsp_decode_i8', 'ser_rsp_send_int'],
	'uint8_t': ['ser_rsp_decode_u8', 'ser_rsp_send_uint'],
	'int16_t': ['ser_rsp_decode_i16', 'ser_rsp_send_int'],
	'uint16_t': ['ser_rsp_decode_u16', 'ser_rsp_send_uint'],
	'int32_t': ['ser_rsp_decode_i32', 'ser_rsp_send_int'],
	'uint32_t': ['ser_rsp_decode_u32', 'ser_rsp_send_uint'],
	'int64_t': ['ser_rsp_decode_i64', 'ser_rsp_send_int64'],
	'uint64_t': ['ser_rsp_decode_u64', 'ser_rsp_send_uint64']
};
...
let paramTemplates = {
	...
	'ret F*': {
		...
		resStructField: p => `${p.shortType} *${p.name};`,
		rspFuncDec: p => `_res->${p.name} = ${p.decoderName}(_value);`,
	...
	'in T[]': {
		...
		recvFuncLoc: p => `${p.type} ${p.name};`,
		recvFuncDec: p => `${p.name} = ser_decode_buffer_into_scratchpad(&_scratchpad);`,
		recvFuncPar: p => `${p.name}`,
	},
	'out T[]?': 'out T[]',
	'out T[]': {
		...
		recvFuncLoc: p => `${p.type} ${p.name};`,
		recvFuncDec: p => `${p.name} = ser_scratchpad_add(&_scratchpad, ${p.basicType.sizeOf} * ${p.getArraySize()});`,
		recvFuncPar: p => `${p.name}`,
		recvFuncBufMax: p => 5,
	...
	'in STR': {
		...
		recvFuncLoc: p => `${p.type} ${p.name};`,
		recvFuncDec: p => `${p.name} = ser_decode_str_into_scratchpad(&_scratchpad);`,
		recvFuncPar: p => `${p.name}`,
	},
	'out STR[]': {
		sendFuncAssign: p => `_result.${p.name} = ${p.name};`,
		...
		recvFuncOutDec: p => `${p.name} = ser_scratchpad_add(&_scratchpad, ${p.getArraySize()});`,
		recvFuncPar: p => `${p.name}`,
	...
	'out RS[]': {
		...
		resStructField: p => `${p.type} ${p.name};`,
		recvFuncLoc: p => `${p.type} ${p.name};`,
		recvFuncOutDec: p => `${p.name} = ser_scratchpad_add(&_scratchpad, ${p.getArraySize()} * sizeof(${p.shortType}));`,
		recvFuncPar: p => `${p.name}`,
		recvFuncBufMax: p => 5,
	...
	'out RS*': {
		...
		recvFuncPar: p => `${p.name}`,
		recvFuncBufMax: p => 3,
		recvFuncCalc: p => `_buffer_size_max += sizeof(${p.shortType});`,
		recvFuncEnc: p => `ser_encode_buffer(&_ctx.encoder, ${p.name}, sizeof(${p.shortType}));`,
	},
	'in S*?': {
	...
	'in S[]': {
		...
		recvFuncDec: p => `${p.name} = ser_scratchpad_add(&_scratchpad, ${p.getArraySize()} * sizeof(${p.shortType}));
				   if (${p.name} == NULL) {
				   @	goto decoding_error;
				   }
	...
	'in CS': {
		sendFuncBufMax: p => 3,
		sendFuncEnc: p => `ser_encode_callback_call(&_ctx.encoder, ${p.name});`,
		recvFuncLoc: p => `${p.base.callbackType} ${p.name};`,
		recvFuncDec: p => `${p.name} = (${p.base.callbackType})ser_decode_callback_call(_value);`,
	},
};
...
let fieldTemplates = {
	...
	'F*': {
		encBufMax: p => parseInt(p.bufferSize),
		encFunc: p => `${p.encoderName}(_encoder, _data->${p.name});`,
		decFunc: p => `_data->${p.name} = ${p.decoderName}(_value);`,
	},
	'S*?': {
		encCalc: p => `_buffer_size_max += (_data->${p.name} == NULL) ? 1 : ${p.encoder.bufferConst ? parseInt(p.encoder.bufferConst) : 0}${p.encoder.bufSizeFunc ? ` + ${p.encoder.bufSizeFunc.name}(_data->${p.name})` : ''};`,
		scratchpad: p => `_scratchpad_size += (_data->${p.name} == NULL) ? 0 : sizeof(${p.shortType})${p.encoder.bufSizeFunc ? ` + ${p.encoder.spSizeFunc.name}(_data->${p.name})` : ''};`,
		encFunc: p => `if (_data->${p.name} == NULL) {\n@ser_encode_null(_encoder);\n} else {\n@${p.encoder.encFunc.name}(_encoder, _data->${p.name});\n}`,
		decFunc: p => !p.encoder.decFunc ? REGEN_WARNING : `if (ser_decode_is_null(_value)) {\n@_data->${p.name} = NULL;\n@ser_decode_skip(_value);\n} else {\n@_data->${p.name} = ser_scratchpad_add(_scratchpad, sizeof(${p.shortType}));\n@${p.encoder.decFunc.name}(${p.encoder.spSizeFunc ? '&_scratchpad' : '_value'}, _data->${p.name});\n}`,
	},
	'RS*?': 'RS*',
	'RS*': {
		...
		decFunc: p => `_data->${p.name} = ser_decode_buffer_into_scratchpad(_scratchpad);`,
	},
	'T[]': {
		encBufMax: p => 5,
		...
		if (this.isString) {
			...
			if (base.isCallback && this.name == 'callback_slot') {
				...
				if (this.isCallback) {
					...
					templName += 'S';
					if (this.dir == Units.IN || this.dir == Units.INOUT || isField) {
...
class GenFunc {
	...
	_getPredefinedResponseHandler() {
		...
		if (!base.params._result) {
			return ['ser_rsp_decode_void', 'ser_rsp_send_void'];
		}
