...
function parseSideData(sideData) {
	...
	if (!(buf[0] in numberToName)) {
		throw Error(`Checksum of autogenerated code is invalid.`);
	}
	return {
...
function validate(code, sideData) {
	...
	if (info.crc != crc) {
		throw Error(`Checksum of autogenerated code is invalid. Expected ${crc} [${createSideData(info.type, crc)}], Current ${info.crc} [${sideData}]`);
	}
	return info.type;
}
